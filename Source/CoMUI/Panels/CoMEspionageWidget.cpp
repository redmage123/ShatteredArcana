...
void UCoMEspionageWidget::SetWizardId(int32 WizardId)
{
	...
	// Pull live agents from the espionage subsystem. Falls back to a
	// "no agents yet" placeholder when the wizard hasn't recruited.
	...
	if (GI && WizardId >= 0)
	{
		...
		{
			for (const FCoMSpyAgent& A : Esp->GetAgents(WizardId))
			{
				FAgentEntry E;
				E.Name = A.AgentName.IsEmpty()
					? FString::Printf(TEXT("Agent #%d (Skill %d)"), A.AgentId, A.SkillLevel)
					: FString::Printf(TEXT("%s (Skill %d)"), *A.AgentName, A.SkillLevel);
				const TCHAR* MissionTag = TEXT("Idle");
				switch (A.CurrentMission)
				{
				case ECoMAgentMission::Spy:                 MissionTag = TEXT("Spying"); break;
				case ECoMAgentMission::Sabotage:            MissionTag = TEXT("Sabotage"); break;
				case ECoMAgentMission::Assassinate:         MissionTag = TEXT("Assassinate"); break;
				case ECoMAgentMission::Steal:               MissionTag = TEXT("Stealing"); break;
				case ECoMAgentMission::Recruit:             MissionTag = TEXT("Recruiting"); break;
				case ECoMAgentMission::GuardAsset:          MissionTag = TEXT("Counterintel"); break;
				case ECoMAgentMission::InfiltrateCity:      MissionTag = TEXT("Infiltrating"); break;
				case ECoMAgentMission::CorruptOfficial:     MissionTag = TEXT("Corrupting"); break;
				case ECoMAgentMission::PropagandaCampaign:  MissionTag = TEXT("Propaganda"); break;
				default: break;
				}
				E.Location = (A.CurrentMission == ECoMAgentMission::MAX)
					? FString(TEXT("Idle"))
					: FString::Printf(TEXT("Target wiz %d (%d turn(s) left)"),
					                  A.TargetWizardId, A.MissionTurnsLeft);
				E.Status = A.bCaptured ? TEXT("Compromised")
				           : A.bDoubleAgent ? TEXT("Double Agent")
				           : MissionTag;
				Agents.Add(E);
			}
		}
	...
	{
		FAgentEntry E;
		E.Name = TEXT("(No agents yet — click Deploy Agent)");
		E.Location = TEXT("—");
		E.Status   = TEXT("—");
		Agents.Add(E);
	}
...
namespace
{
	// Pull the current turn from the turn subsystem; default 1 if missing.
	int32 CurrentTurnOr1(UGameInstance* GI)
	{
		if (GI)
		{
			if (UCoMTurnSubsystem* T = GI->GetSubsystem<UCoMTurnSubsystem>())
			{
				return T->GetCurrentTurn();
			}
		}
		return 1;
	}

	// Pick the first idle agent (CurrentMission == MAX). Returns -1 if none.
	int32 FindIdleAgentId(UCoMEspionageSubsystem* Esp, int32 WizardId)
	{
		if (!Esp) return -1;
		for (const FCoMSpyAgent& A : Esp->GetAgents(WizardId))
		{
			if (A.CurrentMission == ECoMAgentMission::MAX && !A.bCaptured)
			{
				return A.AgentId;
			}
		}
	...
	// Pick a target rival the player has met (or any wizard 0..13 != self).
	int32 PickRivalTarget(UCoMDiplomacySubsystem* Dip, int32 SelfId)
	{
		if (Dip)
		{
			for (int32 Other = 0; Other < 14; ++Other)
			{
				...
				if (Dip->HaveMet(SelfId, Other)) return Other;
			}
		}
		for (int32 Other = 0; Other < 14; ++Other)
		{
			if (Other != SelfId) return Other;
		}
		return -1;
	}
}
...
{
	UGameInstance* GI = GetGameInstance();
	if (!GI || CurrentWizardId < 0) return;
	if (UCoMEspionageSubsystem* Esp = GI->GetSubsystem<UCoMEspionageSubsystem>())
	{
		// Skill 20 baseline ("trained" agent); name generated by the
		// subsystem if AgentName is empty.
		Esp->RecruitAgent(CurrentWizardId, FString(), 20, CurrentTurnOr1(GI));
		SetWizardId(CurrentWizardId); // re-render
	}
}
...
void UCoMEspionageWidget::OnSabotageClicked()
{
	UGameInstance* GI = GetGameInstance();
	if (!GI || CurrentWizardId < 0) return;
	UCoMEspionageSubsystem* Esp = GI->GetSubsystem<UCoMEspionageSubsystem>();
	UCoMDiplomacySubsystem* Dip = GI->GetSubsystem<UCoMDiplomacySubsystem>();
	const int32 AgentId = FindIdleAgentId(Esp, CurrentWizardId);
	const int32 Target  = PickRivalTarget(Dip, CurrentWizardId);
	if (AgentId >= 0 && Target >= 0 && Esp)
	{
		Esp->AssignMission(AgentId, ECoMAgentMission::Sabotage, Target, FIntPoint(0, 0));
...
void UCoMEspionageWidget::OnTheftClicked()
{
	...
	if (AgentId >= 0 && Target >= 0 && Esp)
	{
		Esp->AssignMission(AgentId, ECoMAgentMission::Steal, Target, FIntPoint(0, 0));
		SetWizardId(CurrentWizardId);
	}
}
...
void UCoMEspionageWidget::OnAssassinationClicked()
{
	...
	if (AgentId >= 0 && Target >= 0 && Esp)
	{
		Esp->AssignMission(AgentId, ECoMAgentMission::Assassinate, Target, FIntPoint(0, 0));
		SetWizardId(CurrentWizardId);
	}
}
...
{
	// "Turn Agent" in this UI = propaganda mission (closest analogue to
	// flipping an enemy organisation). Real double-agent flipping happens
	// in the counterintel flow when we capture an enemy spy.
	UGameInstance* GI = GetGameInstance();
	if (!GI || CurrentWizardId < 0) return;
	UCoMEspionageSubsystem* Esp = GI->GetSubsystem<UCoMEspionageSubsystem>();
	UCoMDiplomacySubsystem* Dip = GI->GetSubsystem<UCoMDiplomacySubsystem>();
	...
	if (AgentId >= 0 && Target >= 0 && Esp)
	{
		Esp->AssignMission(AgentId, ECoMAgentMission::PropagandaCampaign, Target, FIntPoint(0, 0));
		SetWizardId(CurrentWizardId);
	}
}
