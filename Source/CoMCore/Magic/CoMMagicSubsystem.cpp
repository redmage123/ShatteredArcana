...
void UCoMMagicSubsystem::ResolveSpell(FCoMSpellCast& Cast)
{
    ...
    case ECoMSpellEffect::Summon:
    {
        ...
        // Prefer the explicit binding on the spell (one spell → one creature).
        // Fall back to the legacy realm+tier switch for spells that haven't
        // been migrated yet -- including non-CoM realms (Spirit/Binding/
        // Glamour) which don't have their own summon rosters.
        FName SummonSpec = Info.SummonSpecID;
        if (SummonSpec.IsNone())
        {
