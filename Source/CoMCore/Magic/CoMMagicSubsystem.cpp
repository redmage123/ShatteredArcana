...
bool UCoMMagicSubsystem::BeginRitual(int32 WizardId, FName RitualId, int32 ManaChannelPerTurn)
{
    ...
    Ritual.CasterWizardId = WizardId;
    Ritual.TotalManaCost = 500; // Would come from data asset
    Ritual.ManaInvested = 0;
    Ritual.ManaPerTurn = ManaChannelPerTurn;
    Ritual.StartTurn = 0;
...
void UCoMMagicSubsystem::CancelRitual(int32 WizardId, FName RitualId)
{
    ...
    {
        return R.CasterWizardId == WizardId && R.RitualId == RitualId;
    });
...
bool UCoMMagicSubsystem::InterruptRitual(int32 WizardId, FName RitualId)
{
    ...
    {
        if (ActiveRituals[i].CasterWizardId == WizardId &&
            ActiveRituals[i].RitualId == RitualId &&
            ActiveRituals[i].bInterruptible)
        {
...
void UCoMMagicSubsystem::ProcessTurn(int32 CurrentTurn)
{
    ...
    for (int32 i = ActiveRituals.Num() - 1; i >= 0; --i)
    {
        ...
        FCoMWizardMagicState& State = GetWizardMagic(Ritual.CasterWizardId);

        int32 ChannelAmount = FMath::Min(Ritual.ManaPerTurn, State.CurrentMana);
        State.CurrentMana -= ChannelAmount;
        Ritual.ManaInvested += ChannelAmount;

        if (Ritual.ManaInvested >= Ritual.TotalManaCost)
        {
